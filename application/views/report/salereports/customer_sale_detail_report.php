...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <br><br>
      <h3 style="color:green;text-align: center;">Customer Sale Detail</h3>
      ...
      <div style="padding-top:-0.1em; font-size:12px; font-weight:600; text-align:center;">Period of <?php echo date('M d, Y', strtotime($this->input->get('StartDate'))).' &nbsp; - &nbsp;  '. date('M d, Y', strtotime($this->input->get('EndDate'))); ?>
        ...
        <ul class="dropdown-menu">
          <li><a href="<?php echo base_url() ?>Export/ExportExcelCustomerSaleDetailReport?StartDate=<?php echo $this->input->get('StartDate') ?>&EndDate=<?php echo $this->input->get('EndDate'); ?>">Excel</a></li>
          <li><a href="<?php echo base_url()?>Export/ExportWordCustomerSaleDetailReport?StartDate=<?php echo $this->input->get('StartDate') ?>&EndDate=<?php echo $this->input->get('EndDate'); ?>">Word</a></li></ul>
          </span> &nbsp; </span>
      <hr style="color: #003eff; border-width: 1px; border-style: inset; display: block;">
		...
		<tr style="font-size:12px; font-weight:600;">
		 ...
		 <td style="text-align:right;"><?php echo number_format($TotalGSTAmount,2); ?></td>
...
<div class="form-group">
          ...
          <div class="col-sm-8">
                <input type="text" name="Subject" id="Subject" class="form-control" required="required" value="Customer Sale Report from: <?php echo date('M d, Y', strtotime($this->input->get('StartDate')));?> To <?php echo date('M d, Y', strtotime($this->input->get('EndDate')));?> ">
          </div>
        </div> <br /><br />
...
<script>
  $(function(){
    $("#Submit").on('click', function(){
      ...
      $.ajax({
        url: '<?php echo base_url(); ?>SendEmail/SendCustomerSaleDetailReport',
        data: {EmployeeId:EmployeeId,OtherEmployee:OtherEmployee,Subject:Subject,Comments:Comments,Message:Message,StartDate:StartDate,EndDate:EndDate,CustomerId:CustomerId},
        type: 'post',
        dataType: 'html',
