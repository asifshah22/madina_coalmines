...
<body class="hold-transition skin-blue sidebar-mini">
  <div class="wrapper">
    ...
    <br><br>
      <h3 style="color:green;text-align: center;">Bank Book</h3>
      ...
      <div style="padding-top:-0.1em; font-size:12px; font-weight:600; text-align:center;">Period of <?php echo date('M d, Y', strtotime($this->input->get('StartDate'))).' &nbsp; - &nbsp;  '. date('M d, Y', strtotime($this->input->get('EndDate'))); ?>
        ...
        <ul class="dropdown-menu">
          <li><a href="<?php echo base_url() ?>Export/ExportExcelBankBookReport?StartDate=<?php echo $this->input->get('StartDate') ?>&EndDate=<?php echo $this->input->get('EndDate'); ?>">Excel</a></li>
          <li><a href="<?php echo base_url()?>Export/ExportWordBankBookReport?StartDate=<?php echo $this->input->get('StartDate') ?>&EndDate=<?php echo $this->input->get('EndDate'); ?>">Word</a></li></ul>
          </span> &nbsp; </span>
      ...
      <div style="height:30px;"></div>
	    ...
	    $NewDetail = '';
            ...
            if(isset($BankBookReport)) {
            foreach($BankBookReport as $BankBookReportRecord) {
            
            $ChartOfAccountsCategoryId = $BankBookReportRecord['ChartOfAccountsCategoryId'];
          //  $PurchaseId = $BankBookReportRecord['PurchaseId'];
	    $ChartOfAccountsId = $BankBookReportRecord['ChartOfAccountsId'];
            $ChartOfAccountsTitle = $BankBookReportRecord['ChartOfAccountsTitle'];
            $ChartOfAccountsCode = $BankBookReportRecord['ChartOfAccountsCode'];
            $DebitIncrease = $BankBookReportRecord['DebitIncrease'];
	    $CreditIncrease = $BankBookReportRecord['CreditIncrease'];
    ...
    {  
        $dBalance = $OpenningBalanceOneRecord['Balance']; 
            ...
            <table border="1" cellspacing="0" cellpadding="3" style="width:100%; text-align: center; padding-left: 1px;">
              <thead>
                <tr>
                 <th style="font-family:Tahoma, Arial; font-size:15px; padding-left:10px;" align="left" colspan="7">General BankBook - <?php echo $ChartOfAccountsTitle; ?></th>
    </tr>
                <tr>
                ...
                <tr>
                 ...
                 <th style="border-bottom:1px solid; width:5%; font-family:Tahoma, Arial; font-weight:bold; font-size:12px; text-align:center;">Voucher</th>
              ...
              foreach($SubBankBookReport as $SubBankBookReportRecord) {
             
              //if($SubBankBookReportRecord['CompanyName'] != '')
            //  {
            
              if($SubBankBookReportRecord['ChartOfAccountsId'] == $BankBookReportRecord['ChartOfAccountsId']) 
              {
                              
              $ChartOfAccountsId = $SubBankBookReportRecord['ChartOfAccountsId'];
              $GeneralJournalId = $SubBankBookReportRecord['GeneralJournalId'];
              $ReferencePrefix = $SubBankBookReportRecord['ReferencePrefix'];
	      $Reference = $SubBankBookReportRecord['Reference'];
              $dTransactionDate = $SubBankBookReportRecord['TransactionDate'];
              $sTransactionDate = date("M d, Y", strtotime($dTransactionDate));
              $Detail = $SubBankBookReportRecord['Detail'];
              //$SalesInvoiceId = $SubBankBookReportRecord['Detail'];
        $SalesInvoiceId = $SubBankBookReportRecord['SalesInvoiceId'];
        $PurchaseId = $SubBankBookReportRecord['PurchaseId'];
              $dDebit = $SubBankBookReportRecord['Debit'];
              $dCredit = $SubBankBookReportRecord['Credit'];
              $CustomerName = $SubBankBookReportRecord['CustomerName'];
              $VendorName = $SubBankBookReportRecord['VendorName'];
              $AccountTitle = $SubBankBookReportRecord['AccountTitle'];
        //$InvoicePaidStatus = $SubBankBookReportRecord['InvoicePaidStatus'];
        $voucherType = $SubBankBookReportRecord['VoucherType'];
              $ReconciliationStatus  = $SubBankBookReportRecord['ReconciliationStatus'];
        ...
        */
              ...
              <tr style="font-size:12px;">
                ...
                <td style="text-align: left; padding-left:10px;">
                  <?php echo $SubBankBookReportRecord['CustomerName']; ?>
                  <?php echo $SubBankBookReportRecord['VendorName']; ?>
                  <?php echo $SubBankBookReportRecord['AccountTitle']; ?>    
                </td>        
                <td style="text-align: left; padding-left:10px;">
                <?php
...
//    }
    ...
    echo $NewDetail;
                ...
                ?></td> 
                <td style=" text-align:left;"><?php echo '<a href="javascript:void(0)" onclick="window.open(\''.base_url().'Generaljournal/ViewVoucher/'.$GeneralJournalId.'\',\'popUpWindow\',\'height=450,width=800,left=400,top=100,resizable=no,scrollbars=0,toolbar=no,menubar=no,location=no,directories=no, status=yes\');">'.$ReferencePrefix.''.$Reference.'</a>'; ?></td>
        ...
        $dBalance = 0;
            ...
            ?> 
              <!--  <tr style="font-size:13px;">
                ...
                <td style="font-weight: 700; text-align: center;"><?php // echo number_format($dGrandTotal_Debit,0)2 ?></td>
                <td style="font-weight: 700; text-align: center;"><?php // echo number_format($dGrandTotal_Credit,0)2 ?></td>
                <td style="font-weight: 700; text-align: center;"><?php // echo number_format($GrandBalance,0)2 ?></td>
              </tr>
              </tbody> -->
              <tr>
                  <td colspan="7">&nbsp;</td>
...
<div class="form-group">
          ...
          <div class="col-sm-8">
                <input type="text" name="Subject" id="Subject" class="form-control" required="required" value="General BankBook Report from: <?php echo date('M d, Y', strtotime($this->input->get('StartDate')));?> To <?php echo date('M d, Y', strtotime($this->input->get('EndDate')));?> ">
          </div>
        </div> <br /><br />
...
<script>
  $(function(){
    $("#Submit").on('click', function(){
      ...
      $.ajax({
        url: '<?php echo base_url(); ?>SendEmail/SendBankBookReport',
