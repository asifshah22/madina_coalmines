...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <br><br>
      ...
      <div style="padding-top:-0.1em; font-size:12px; font-weight:600; text-align:center;">Period of <?php echo date('d M,  Y'); ?>
      </div>
        ...
        <span class="dropdown pull-left">
          <a class="dropdown-toggle" data-toggle="dropdown" href="#" style="color:white"><span class="caret"></a> &nbsp;
        <ul class="dropdown-menu">
          <li><a href="<?php echo base_url() ?>Export/ExportExcelChartOfAccountReport?CategoryId=<?php echo $this->input->get('CategoryId') ?>&ControlCodeId=<?php echo $this->input->get('ControlCodeId'); ?>">Excel</a></li>
<!--          <li><a href="<?php // echo base_url()?>Export/ExportWordSaleDetailReport?StartDate=<?php // echo $this->input->get('StartDate') ?>&EndDate=<?php // echo $this->input->get('EndDate'); ?>">Word</a></li> -->
      ...
      <!--  <button class="pull-right" type="button" id="SendMail" data-toggle="modal" data-target="#myModal">Send as Email</button> -->
            ...
            <table class="table table-bordered table-basic">
              <thead>
                <tr style="background:#205081; font-size:13px; color:#FFFFFF;">
                 <th style="text-align: center;">COA ID</th>
		...
		if(isset($GetAllChartOfAccount)) {
		    $creaditTotal=0;
		    $debitTotal=0;
		foreach($GetAllChartOfAccount as $ChartOfAccountsReport)
		{
		  //  print_r($ChartOfAccountsReport);
		?>
	    <tr style="font-size:12px;">
    		<td style="text-align: center;"><?php echo $ChartOfAccountsReport['ChartOfAccountCode']; ?></td>
		 	...
		 	<td style="text-align:left;"><?php echo $ChartOfAccountsReport['ChartOfAccountTitle']; ?></td>
		 	<td style="text-align:left;"><?php 
		 	if($ChartOfAccountsReport['CategoryName']!='Liabilities' && $ChartOfAccountsReport['CategoryName']!='Income' && $ChartOfAccountsReport['CategoryName']!='Equity/Capital'){
		 	    
		 	    $debit =$ChartOfAccountsReport['TotalDebit']-$ChartOfAccountsReport['TotalCredit'];
		 	  echo  number_format($debit);
		 	  $debitTotal+=$debit;
		 	}
		 	 ?></td>
		 	<td style="text-align:left;"><?php 
		 		if($ChartOfAccountsReport['CategoryName']!='Assets' && $ChartOfAccountsReport['CategoryName']!='Expenses'){
		 	  $credit= $ChartOfAccountsReport['TotalCredit']-$ChartOfAccountsReport['TotalDebit'];
		...
		<tr style="font-size:13px;">
		 	...
		 	<td style="text-align:left;"><b><?php 
		 		echo number_format($debitTotal);
		 	 ?></b></td>
		 	<td style="text-align:left;"><b><?php 
		 	echo number_format($creaditTotal);
		 	?></b>
		 	</td>
        </tr>
		<?php
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="hidden" name="CategoryId" id="CategoryId" value="<?php echo $this->input->get('CategoryId'); ?>">
<div class="form-group">
          <label for="Subject" class="col-sm-3 control-label"><b>Subject:</b></label>
          <div class="col-sm-8">
                <input type="text" name="Subject" id="Subject" class="form-control" required="required" value="Chart of Account Report">
          </div>
        </div> <br /><br />
...
<script>
  $(function(){
    $("#Submit").on('click', function(){
      ...
      $.ajax({
        url: '<?php echo base_url(); ?>SendEmail/SendChartOfAccountReport',
        data: {EmployeeId:EmployeeId,OtherEmployee:OtherEmployee,Subject:Subject,Comments:Comments,Message:Message,CategoryId:CategoryId},
        type: 'post',
        dataType: 'html',
