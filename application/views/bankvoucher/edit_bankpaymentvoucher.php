  ...
  <div class="content-wrapper">
    ...
    <section class="content">
	 ...
	 <div class="box box-info">
	     ...
	     <h3 class="box-title text-light-blue"><span style="color:#000000;">Update Bank Payment Voucher</span></h3>
         </div>
	
         <form role="form" id="GeneralJournalForm" method="post" action='<?php echo site_url("BankPaymentVoucher/UpdateBPV")?>'>
	 <input type="hidden" name="GeneralJournalId" value="<?php echo $BankPayment->GeneralJournalId; ?>">    
	  <div class="box-body">
	  <div class="row invoice-info">
	    <div class="col-sm-2 form-group">
	      <strong>Voucher #:</strong><br>
		<?php echo $BankPayment->ReferencePrefix.''.$BankPayment->Reference; ?>
            </div>
	    <div class="col-sm-3 form-group">
	      <strong>Voucher Date:</strong><br>
		<div class="input-group date">
		 <input style="width:220px; height:30px;" class="form-control" name="TransactionDate" value="<?php echo date("m/d/Y", strtotime($BankPayment->TransactionDate)); ?>" id="datepicker1" type="text" required placeholder="Enter Transaction Date" style="width:37%;" autocomplete="off">
		</div>
	    </div>
	    <div class="col-sm-3 form-group">
	      <strong>Bank Account:
		 ...
		 <option selected="selected" value="">Select Bank Account</option>
		 <?php foreach ($GetBankAccounts as $BankAccount) { ?>
		 <option value="<?php echo $BankAccount['AccountId'].'-'.$BankAccount['ChartOfAccountsId']; ?>"<?php if($BankPayment->BankAccountId == $BankAccount['AccountId']) echo "selected=selected"; ?>><?php echo $BankAccount['AccountTitle'] . " " . $BankAccount['AccountNumber'];?></option>
		 <?php } ?>
	        </select>
	    </div>
	    <div class="col-sm-3">
		...
		<div class="input-group date">
		   <select name="ChequeNumber" id="ChequeNumber" class="form-control select2" style="width:220px;">
		    <option selected="selected" value="<?php echo $BankPayment->ChequeBookId.'-'.$BankPayment->ChequeNumber; ?>"><?php echo $BankPayment->ChequeNumber != 0 ? $BankPayment->ChequeNumber : 'Without Cheque'; ?></option>
		    <?php
		    $keys = array_keys($ChequeLeaves);
		    for($i = 0; $i < count($ChequeLeaves); $i++) {     
		    foreach($ChequeLeaves[$keys[$i]] as $key => $value) {
		    ?>
	  ...
	  <!-- Voucher Detail Entries Block -->
		...
		<table id="GJ_EntriesTable" style="border: 1px; solid" class="table table-bordered text-center">
		   ...
		   <?php
		    ...
		    foreach($BankPaymentEntries as $Record)
		    {
		  ...
		  <td>
		   ...
		   <td><input style="width:100%; text-align:right;" type="text" id="Debit_<?php echo $SNo; ?>" name="Debit[]" value="<?php echo $Record['Debit'] == '0.00' ? '' : $Record['Debit']; ?>" autocomplete="off"></td>
		   <td>
		     <?php if($SNo != 1) { ?>   
		...
		<tr>
		 ...
		 <td style="text-align:right; width:23%; padding:9px; padding-right:3px;  font-weight:600;"><div id="TotalAmount"><?php echo number_format($BankPayment->TotalDebit,2); ?></div></td>
	  ...
	  <div class="box-body">
		...
		<label for="VoucherNote" class="col-sm-2 control-label">Voucher Note:</label>
		 <div class="input-group">
		  <textarea name="VoucherNote" class="form-control" rows="2" cols="60"> <?php echo $BankPayment->VoucherNote; ?></textarea>
		 </div>
	      </address>
	     </div>
	     <div class="col-md-2">
