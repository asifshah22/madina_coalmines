...
<?php $this->load->view('includes/sidebar'); ?>
  ...
  <div class="content-wrapper">
        ...
        <div class="col-md-12">
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title text-light-blue">Edit Sales Return</h3>
            </div>
      <?php echo "<font style=color:#00468E; font-weight:bold text-align:center;>".$this->session->flashdata('record_update')."</font>"; ?>
	<form class="form-horizontal" method="post" action="<?php echo base_url("SalesReturn/UpdateSalesReturn"); ?>" name="basic_validate" id="basic_validate" novalidate="novalidate">
	    <input type="hidden" name="SaleReturnId" value="<?php echo $SalesReturn->SaleReturnId; ?>">
	 <input type="hidden" name="SaleReturnNo" value="<?php echo $SalesReturn->SaleReturnNo; ?>">
      ...
      <div class="col-md-4 invoice-col">
        ...
        <strong>Sale Return #:</strong><br>
          <span style="color:#3333CC; font-size:13px; font-weight:bold;"><?= $SalesReturn->SaleReturnId; ?></span>
        </address>
      </div>

      <div class="col-md-4 invoice-col">
        <address>
        <strong>Select Bank Account:</strong><br>
          <select name="BankAccountId" id="BankAccountId" class="form-control select2" style="width: 215px;">
            ...
            ?>
            <option <?php if($SalesReturn->AccountId == $BankAccounts['AccountId']) { echo 'selected=selected'; }  else{ ''; } ?> value="<?php echo $BankAccounts['AccountId'] . "-" .$BankAccounts['ChartOfAccountId']; ?>"><?php echo $BankAccounts['AccountTitle'] . " - " . $BankAccounts['AccountNumber'];  ?></option>
            <?php
            } ?>
          </select>
        </address>
      ...
      <div class="col-md-4 invoice-col">
        ...
        <strong>Sale Return Date:</strong><br>
          <input class="form-control" id="datepicker1" type="text" name="SaleReturnDate" value="<?php echo date("m/d/Y", strtotime($SalesReturn->SaleReturnDate)); ?>" autocomplete="off" style="width: 215px;">
        </address>
      </div>
      ...
      <div class="col-md-4 invoice-col">
        ...
        <strong>Sale Return Type:</strong><br>
          ...
          ?>
			...
			<input type="hidden" name="SaleReturnType"  value="<?php echo $SalesReturn->SaleReturnType; ?>">

      <div class="col-md-4 invoice-col">
        <address>
        <strong>Customer:</strong><br>
          <select name="CustomerId" id="CustomerId" style="width:215px; margin-top:1px;" class="form-control select2">
            <option selected="selected" value="0">Select Customer</option>
             <?php foreach ($AllCustomers as $CustomerRecord) { ?>
            <option value="<?php echo $CustomerRecord['CustomerId'].'-'.$CustomerRecord['ChartOfAccountId']; ?>"<?php if($CustomerRecord['CustomerId'] == $SalesReturn->CustomerId) echo "selected=selected"; ?>><?php echo $CustomerRecord['CustomerName'];?></option>
             <?php } ?>
          </select>
        </address>
      </div>

      <div class="col-md-4 invoice-col" style="border: 0px solid;">
        <address>
        <strong>Transportation:</strong><br>
          <select name="ReferenceId" id="ReferenceId" style="width:215px; margin-top:1px;" class="form-control select2">
           ...
           <?php foreach ($AllReferences as $ReferenceRecord) { ?>
            <option value="<?php echo $ReferenceRecord['ReferenceId']; ?>"<?php if($ReferenceRecord['ReferenceId'] == $SalesReturn->ReferenceId) echo "selected=selected"; ?>><?php echo $ReferenceRecord['FullName'];?></option>
           <?php } ?>
          </select>
        </address>
      </div>
      ...
      <div class="col-md-4 invoice-col">
        ...
        <strong>Sale Return Note:</strong><br>
            <input type="text" name="SaleReturnNote" id="SaleNote" class="form-control" style="width: 215px;" value="<?php echo $SalesReturn->SaleReturnNote; ?>">
        </address>
      </div>     
      ...
      <address>
        ...
        ?>
        <option <?php if($SalesReturn->SalemanId == $AllSaleman['SalemanId']) { echo 'selected=selected'; } else { echo ''; } ?> value="<?php echo $AllSaleman['SalemanId'].'-'.$AllSaleman['ChartOfAccountId']; ?>"><?php echo $AllSaleman['SalemanName'];  ?></option>
        <?php
        } ?>
        </select>
      </address>
    ...
    <th style="padding:5px; width:3%;"><i class="fa fa-trash"></i></th>
	       </tr>
               </thead>
	        <?php 	
		//echo '<pre>';
		//print_r($SalesReturnDetail);
		//die;
		...
		<tr class="txtMult">
		 ...
		 <td>
		  <input style='width:130px;margin-top:1px;' type='text' name="ProductName[]" id="ProductName_<?php echo $SNo?>" autocomplete="off"value="<?php echo $Record['ProductName'];?>">
     ...
     <input type='hidden' id="hdnColourName_<?php //echo $SNo?>" name="ColourId[]" value="<?php //echo $Record['ColourId'];?>" >
		 ...
		 <td><input style='width:90%; margin-top:1px;' type='text' id="Rate<?php print $SNo; ?>" class="Rate" name='Rate[]' autocomplete="off" value="<?php echo $Record['Rate'];?>"></td>
     <td style="text-align:center;"><input style='width:100%;margin-top:1px; text-align:right;' type='text' id="Quantity<?php print $SNo; ?>" class="Quantity" name='Quantity[]' autocomplete="off" value="<?php echo $Record['Quantity'];?>"></td>
		 <input type='hidden' id="OldQuantity<?php print $SNo; ?>" class="Quantity" name='OldQuantity[]' autocomplete="off" value="<?php echo $Record['Quantity'];?>">
		 <td><input style='width:90%; margin-top:1px; text-align:right;' type='text' id="Amount<?php print $SNo; ?>" class="Amount" name='Amount[]' value="<?php echo $Record['Amount'];?>"></td>
      <td><input style='width:90%; margin-top:1px; text-align:right;' type='text' id="DiscountAmount<?php print $SNo; ?>" class="DiscountAmount" name='DiscountAmount[]' value="<?php echo $Record['DiscountAmount'];?>"></td>
    <td><input style='width:90%; margin-top:1px; text-align:right;' type='text' id="TaxPercentage<?php print $SNo; ?>" class="TaxPercentage" name='TaxPercentage[]' min="0" value="<?php echo $Record['TaxPercentage'];?>"></td>
     ...
     <td><input style='width:90%; margin-top:1px; text-align:right;' type='text' id="Amount<?php print $SNo; ?>" class="NetAmount" name='NetAmount[]' value="<?php echo $Record['NetAmount'];?>"></td>
		 <td style="width:70px;text-align:right; cursor:pointer; color:red;"><i class="fa fa-trash remove" title="Delete" style="margin-top:1px;"></i></td>
		</tr>
		<?php 
		$SNo++;
...
<div style="height: 50px;"></div>
    ...
    <div class="row">
      <div class="col-md-2">
        <button type="submit" name="AddSaleReturnRecordBtn" value="AddSaleReturnRecord" id="AddRecord" class="btn btn-block btn-primary">Update Record</button>
      </div>
      <div class="col-md-2">
...
<script>
  $(document).ready(function () {
	...
	cols += '<td><input style="width:90%; margin-top:1px; text-align:right;" type="number" id="Rate'+ counter +'" class="Rate" name="Rate[]" autocomplete="off" step="0.01"></td>';
	cols += '<td><input style="width:100%; margin-top:1px; text-align:right;" type="number" id="Quantity'+ counter +'" class="Quantity" name="Quantity[]" autocomplete="off" step="0.01"></td>';
	cols += '<td><input style="width:90%; margin-top:1px; text-align:right;" type="number" id="Amount'+ counter +'" class="Amount" name="Amount[]" step="0.01"></td>';
  cols += '<td><input style="width:90%; margin-top:1px; text-align:right;" type="number" id="DiscountAmount'+ counter +'" class="DiscountAmount" name="DiscountAmount[]" step="0.01"></td>';
  ...
  cols += '<td><input style="width:90%; margin-top:1px; text-align:right;" type="number" id="Amount'+ counter +'" class="NetAmount" name="NetAmount[]" step="0.01"></td>';
	cols += '<td style="width:70px;text-align:right; cursor:pointer; color:red;"><i class="fa fa-trash remove" title="Delete" style="margin-top:1px;"></i></td>';
        ...
        newRow.append(cols);
        $("#SaleReturn_EntriesTable").append(newRow);
        counter++;
    });
    
    // Start Remove row script
    $("#SaleReturn_EntriesTable").on("click", ".remove", function (event) {
